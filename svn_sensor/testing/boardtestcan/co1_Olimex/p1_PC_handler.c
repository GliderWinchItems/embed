...
//u16 USART2_txint_busy()	// Return 0 = buffers are free.
...
"m - Toggle display of CAN msgs for a given ID and data type on-off\n\r",\
...
static short input_check(void)
{
	...
	case 0:	// Here, checking for an input line
		strlb = USART2_rxint_getlineboth();	// Get both char count and pointer (@1)
...
//printf("cmd %c strlb.ct %d\n\r",*strlb.p, strlb.ct); USART2_txint_send();
...
//printf("ccc %c strlb.ct %d\n\r",*strlb.p, strlb.ct); USART2_txint_send();
	...
	case 1:	// Here, echo input line back
		/* Keep going through this until echo line has been sent to USART2 buffers */
	...
	case 2:	// Here, good or bad check
		if (cmd == 0)
		{ // Here invalid
printf(" %c 0x%02x ",cmd_save,cmd_save); USART2_txint_send(); // Let's look more closely at what was typed in.
...
static int print_menu(void)
{
	...
	case 1:
		...
		if (USART2_txint_busy() != 0) return 1;// Return: all buffers full (@1)
		if (line_ct < MENULINES)	// Have we sent the last line?	
		{ // Here, no.  Send another
			USART2_txint_puts ((char*)menu[line_ct]);	USART2_txint_send();// (@1)
...
static short send_line(const char * p)
{
	if (USART2_txint_busy() == 1) return 1;// Return: all buffers full (@1)

	USART2_txint_puts ((char*)p);		USART2_txint_send();	//  (@1)
