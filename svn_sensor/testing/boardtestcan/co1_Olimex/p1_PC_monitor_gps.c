...
u32 canlogct_diff;
	...
	case 1: /* Do a column header */
		...
		if (USART2_txint_busy() == 1) return;		// Return: all buffers full (@1)
		USART2_txint_puts(" GPS time sync data, number of msgs per sec\n\r");
		USART2_txint_send();//  (@1)
		monitor_gps_state = 2;
		break;
	...
	case 2:
		/* Check if there is an available buffer */
		if (USART2_txint_busy() == 1) return;		// Return: all buffers full
...
//printf ("%8u ", diff5);
			...
			printf("\n\r");
			USART2_txint_send();		// Start line sending.
