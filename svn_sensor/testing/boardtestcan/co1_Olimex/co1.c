...
/* Test SD card inserted here. Loop if not sensed. */
	...
	{
		printf("sdlog_init failed.  It returned: %x (%d)\n\r",i,i);  USART2_txint_send();
	}
...
// Should we have "several" LEDs for Uli, e.g. SD OK, GPS OK?
	...
	/* Set filter to accept all loggable unitid & loggable data type msgs */
	can_filtermask16_add( ( 0 | (CAN_EXTRID_MASK >> 16)) );	// Only exclude msgs w extra data field id bits
	can_ret = can_filtermask16_add( 0 );	// Only exclude msgs w extra data field id bits
...
/* -------------------- Endless bliss of logging ----------------------------------------------------- */
	...
	while (1==1)
	{
		...
		/* Poll to log -- HANDLED BY INTERRUPTS */
//		can_log_can();	// Log any ready CAN msgs

		/* Poll for keyboard/PC input and handling in the thereafter. */
		p1_PC_handler();

		/* 1 sec on/off to entertain the hapless op. */
