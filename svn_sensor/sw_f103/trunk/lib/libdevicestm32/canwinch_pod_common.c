/******************** (C) COPYRIGHT 2013 **************************************
* File Name          : canwinch_pod_common.c
* Hackerees          : deh
...
u32	fifo1cycnt;	// DTW module 32b processor cycle counter
 ...
 */
	temp = CAN_MCR(CAN1);			// Get a copy of the MCR
	temp &=  0x1;				// Clear all bits except for INAQ
	temp |=  (bitcvt(p->dbf) << 16);	// Debug freeze
	temp |=  (bitcvt(p->ttcm) << 7);	// Time triggered communication mode
	temp |=  (bitcvt(p->abom) << 6);	// Automatic bus-off management
	temp |=  (bitcvt(p->awum) << 5);	// Auto WakeUp Mode
	temp |=  (bitcvt(p->nart) << 4);	// No Automatic ReTry (0 = retry; non-zero = auto retry)
	CAN_MCR(CAN1) = temp;			// Update MCR
...
static void loadmbx(struct CANRCVBUF *px)
{
	...
	/* Set CAN ID and Request a transmission */
