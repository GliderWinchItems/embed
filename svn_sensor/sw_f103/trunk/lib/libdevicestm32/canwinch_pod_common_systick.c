/******************** (C) COPYRIGHT 2013 **************************************
* File Name          : canwinch_pod_common_systick.c
* Hackerees          : deh
* Date First Issued  : 02/13/2013
* Board              : STM32F103VxT6_pod_mm
* Description        : CAN routines for winch instrumentation--POD board...sensor
*******************************************************************************/
/* 
...
is timed using the SYSTICK counter (24b).  With a nominal 64,000,000 processor clock a
 1/64 sec interval is nominally 1,000,000.  The 24b SYSTICK counter covers this in one
gulp.  Each SYSTICK counter triggers the completion of a measurement for the 1/64th sec
interval and set up a buffer to transmit a CAN message during the following interval.

The phasing of when the 1/64th interval ends|starts is adjusted for the delay that occurs
between the actual 1/64th sec demarcation (with respect to the GPS) and the arrival of
the (high priority) CAN time sync message.
...
#define TICKAVEBITS	1	// Number bits in averaging size
...
u32	systicknominal;		// Nominal number of ticks in 1/64th sec

u32	stk_load;		// SYSTICK reload mirror

u32	stk_val;		// STK_VAL save upon FIFO 1 interrupt
u32	fifo1msgflg;		// Running count of FIFO 1 interrupts
...
static u32	can_ticksper64thdelta;
...
void 	(*systickHIpriority_ptr)(void) = 0;	// SYSTICK handler (very high priority)
...
u32	fifo1cycnt;	// Used for processor ticks per sec measurment

/* Procesor cycle counter is saved upon SYSTICK interrupt entry */
...
/* Prototypes */
void CAN_sync(u32 fifo1cycnt);	// Edit and compute ticks per sec
...
static void SYSTICK_init (void)
{
	...
	systicknominal = (sysclk_freq/64 );	// Nominal ticks per 1/64 sec

	/* Set limits for valid processor ticks between 1/64th CAN msg events */
	can_ticksper64thdelta = (systicknominal / 40);	// Allow +/- 2.5% range
	can_ticksper64thHI = systicknominal + can_ticksper64thdelta;
	can_ticksper64thLO = systicknominal - can_ticksper64thdelta;
...
can_debugP += 1;	// Msgs added to buffer
	...
	return	can_msg_txchkbuff_sys(); // Return number msg remaining buffered
}
...
/*-----------------------------------------------------------------------------
 * void ocphasing1(void);	// CAN FIFO 1 leads OC
...
{ // Here, we have a new STK_VAL to use for phasing
	...
	/* Shift 'zero' by the delay between GPS and reception of CAN msg */
	sval = ((int)stk_val - (int)CAN_TICK_DELAY);
	if (sval < 0)	// Did we wrap?
		sval += STK_LOAD; // Adjust for wrap-around

	/* Are we in the "prev" or "new" interval? (Remember counter counts down.) */
	if (sval > (s32)(systicknominal >> 1)) // Count over 1/2 means in beginning of interval
	{ // Here, closer to the beginning of an interval than the end
		sval = -(STK_LOAD - sval);
	}

CAN_dif = sval;

	/* Set adjustment for phasing, with the "gain" lowered */
	phasing_oneinterval = -sval / 4;
...
void setnextoc(void)
{
	...
	STK_LOAD = (systicknominal + tmp + phasing_oneinterval);	// Set next interval end time (32b)

CAN_dbg1 = STK_LOAD;
CAN_dbg2 = phasing_oneinterval;
...
void SYSTICK_IRQHandler (void)
{
	...
	SCB_CFSR &= ~SCB_ICSR_PENDSTCLR;	// Clear interrupt

	/* Do we have sync msg */
	if (fifo1msgflg != fifo1msgflg_prev)
	{
		fifo1msgflg_prev = fifo1msgflg;
		systickphasing();
	}
	...
	/* Call (very short) routine to close out measurements, if an address is set up */
	if (systickHIpriority_ptr != 0)	// Having no address for the following is bad.
		(*systickHIpriority_ptr)();	// Go, BUT DON'T TARRY.

	/* Trigger a pending interrupt that will handle the computation for sync'ing. */
	NVICISPR(NVIC_I2C2_EV_IRQ);	// Set pending (low priority) interrupt for  ('../lib/libusartstm32/nvicdirect.h')
...
CAN_ave = tickavesum;
		...
		deviation_oneinterval = tmp;	// Interval = whole.fraction	
...
void I2C1_EV_IRQHandler(void)
{
	/* Call routine in 'Tim4_pod_common.c' to handle time */
	CAN_sync(fifo1cycnt);
//while(1==1);
