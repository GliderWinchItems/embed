...
void hs_sock_new_connect(int newsock, int is_server)
{
	...
	LOOP(p)
		if(IS_FREE(p))
		{
			...
			syslog(LOG_INFO, "[%d/%d]hs_sock_new_connect\n", p-ccb, newsock);
			break;
		}
...
static void *in_thread(void *arg)			/* 1 of these for all connects */
{
	...
	while(1==1)
	{
		 ...
		 */
		select(connect_fd_max+1, &recv_fd, NULL, NULL, &timeout);
		select_seqn += 1;
		...
		LOOP(in)							/* Visit all connects */
		{
			...
			if(FD_ISSET(in->socket, &recv_fd))
			{
				 ...
				 */
				tmp = in->ptibe - in->ptibfs;
				n = hs_recv(in->socket, in->ptibfs, tmp);
				if(n > tmp) 
