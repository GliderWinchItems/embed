...
#define	HS_MAX_CONNECT	20			/* Maximum number of clients */
...
void hs_sock_new_client(int newsock)
{
	...
	syslog(LOG_INFO, "hs_sock_new_client(%d) called\n", newsock);
	...
	{
		syslog(LOG_INFO, "hs_sock_new_client(%d) -- too many clients\n", newsock);
		close(newsock);
	}
}
					...
					if(*s++ == ASCII_LF)		/* Full line(s)? */
					{
						...
						LOOP(out)				/* Visit all clients */
						{
							...
							if(in != out)		/* Skip ourselves */
							{
								...
								if(i != j)
									syslog(LOG_INFO, "enqueue err, %d/%d\n", j, i);
