...
/* Log up to 2 messages and hs_exit(EXIT_FAILURE).  Either msg0 or msg1 or both
 * (although this seems unlikely) can be NULL.
 */

int hs_send(int sock, char *buf, int n);
/* Loop until all n chars are read into buf.  Returns 0 if no errors,
...
/* Log code with syslog and exit(code);
 */
