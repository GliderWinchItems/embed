...
int search_highest_pid (int fd, int blkmax)
{
	...
	int delta;
	int low_block = 0;
	int high_block = blkmax-1-SDLOG_EXTRA_BLOCKS;	// index of last highest blk written
	unsigned long long pid;
	unsigned long long pidx;
	unsigned long long id0 = getaPID(fd, low_block);	// write.block.str.pid;
...
int ct=0;
	...
	while(1==1)
	{
		...
		pid = getaPID(fd, high_block);	//sdcard_read(mid_block, write.block.raw);
		if (pid == 0) return 0; // Return: error getting PID  (PID is offset by 1)
...
// ##### added code to the the sdlog.c search #########
			/* Get next block.  If it is higher, then the highest PID is in the half
                           above the current high_block. */
//			pidx = getaPID(fd, high_block+1);	
//			if (pidx > pid)
//				high_block += delta;
		}
pidx = getaPID(fd, mid_block);
printf("pid ct: %3d high_block: %8d low_block: %8d imid: %d pid: %llX \n",++ct,high_block,low_block,mid_block, (pidx-1));
	}
	return 0;
}
/******************************************************************************
...
{
	int blkr = (blk + (blkmax - rotate + 1)); // Allow for extra blocks in wrap-around
...
/******************************************************************************
 * int get1stpkt(int fd, struct LOGBLK* pblk, struct PKTP* pktp, struct CANRCVTIMBUF* pcan);
 * @param	: fd = file descriptor
 * @param	: pblk = pointer to struct with blk number and time
 * @param	: pktp = pointers and stuff
 * @param	: pcan = results in CAN struct format
 * @return	:  0 = OK
 *        	: -1 = error findng first packet within the block
 *		: -2 = read block error
 * Given "logical" block number (meaning highest logical block number is always
...
int ct = 0;
 ...
 printf("tim ct: %3d rmax: %d imin: %d imid: %d rmid: %9d logblk.blk %9d %s",++ct,search_rotate(imax, rotate, blkmax),imin,imid, search_rotate(imid, rotate, blkmax),logblk.blk,ctime(&sr.t));
	}
