...
#include "../../../../svn_discoveryf4/common_all/trunk/USB_PC_gateway.h"
...
void can_write2(FILE* fpOut, struct PKTP *pp)
{
	...
	struct PCTOGATEWAY pctogateway;
	char ww[256];
	unsigned long long tt = *((unsigned long long *)pp->p);	// Get linux time in 1/64th sec ticks

	sprintf(ww,"%010llu |",tt);
	write(fileno(fpOut),ww,strlen(ww));
