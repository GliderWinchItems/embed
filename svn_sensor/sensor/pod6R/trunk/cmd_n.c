...
static struct CANIN* pbase;
static struct CANIN* pend;
static struct CANIN* pwork;
static uint8_t sw_init = 0;
static uint8_t flag_ov = 0;
static uint32_t tik = 0;
static uint32_t canmsgctr = 0;

/* **************************************************************************************
 * int32_t cmd_n_init(uint32_t count);
...
int32_t cmd_n_init(uint32_t count)
{
	...
	DTW_counter_init();	// Use DTW counter for timing
	sw_init = 1;
	pwork = pbase;
	pend  = (pbase + count);
	return 0;
...
void cmd_n(struct CANRCVBUF* p)
{
	...
	{ 
		sw_init = 1;
		cmd_n_init(CANIDTABLESIZEDEFAULT);
	}
