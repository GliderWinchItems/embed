/******************************************************************************
* File Name          : adcsensor_vcal.c
* Date First Issued  : 08/04/2015
* Board              : Olimex
* Description        : ADC routines for f103 Olimex
*******************************************************************************/
/* 
08/04/2015 This is a hack of svn_sensor/sensor/tenion/trunk/adcsensor_tension
*/
...
0 PA3  - IN3 AD7799 temperature
1 PC 0 - IN10 Voltage source temperature
2     - Internal temperature reference
3     - Internal voltage source
4 PC1  - Raw input voltage
5 PC2  - Direct measurement pin
*/
...
#define SMP1	1
#define SMP2	1
#define SMP3	4 // PA3 Thermistor AD7799
#define SMP4	1
#define SMP5	1
#define SMP6	1
#define SMP7	1
#define SMP8	1
#define SMP9	1
#define SMP10	4 // PC0 Thermistor voltage source package
#define SMP11	4 // PC1 2.5v precision voltage source
#define SMP12	4 // PC2 On-board ADC
#define SMP13	1
#define SMP14	1
#define SMP15	1
#define SMP16	4 // Internal temp ref
#define SMP17	7 // Internal voltage ref (Vrefint)


/* ********** Static routines **************************************************/
static void adcsensor_vcal_init(void);
static void adcsensor_vcal_start_conversion(void);
static void adcsensor_vcal_start_cal_register_reset(void);
static void adcsensor_vcal_start_calibration(void);

/* CIC routines buried somewhere below */
static void adc_cic_filtering_ycal(void);
static void adc_cic_init_ycal(void);
...
*******************************************************************************/
const struct PINCONFIGALL pin_therm1 = {(volatile u32 *)GPIOA, 3, IN_ANALOG, 0};
const struct PINCONFIGALL pin_therm2 = {(volatile u32 *)GPIOC, 0, IN_ANALOG, 0};
const struct PINCONFIGALL pin_therm3 = {(volatile u32 *)GPIOC, 1, IN_ANALOG, 0};
const struct PINCONFIGALL pin_therm4 = {(volatile u32 *)GPIOC, 2, IN_ANALOG, 0};
...
static void adcsensor_vcal_init(void)
{
	...
	pinconfig_all( (struct PINCONFIGALL *)&pin_therm1);
	pinconfig_all( (struct PINCONFIGALL *)&pin_therm2);
	pinconfig_all( (struct PINCONFIGALL *)&pin_therm3);
	pinconfig_all( (struct PINCONFIGALL *)&pin_therm4);
	...
	/*               use DMA   |  Continuous   | Power ON      | Vrefint & temp sensor	*/
	ADC1_CR2  = (  ADC_CR2_DMA | ADC_CR2_CONT  | ADC_CR2_ADON  | (1 << 23)); 
	...
	ADC1_SMPR1 = ( (SMP17 << 21) | (SMP16 << 18) | (SMP12 << 6) | (SMP11 << 3) | (SMP10 << 0) );
	ADC1_SMPR2 = ( (SMP3 << 9) );

	/* Setup the number of channels scanned */
	ADC1_SQR1 =  ((NUMBERADCCHANNELS_TEN-1) << 20);	// Regular channel sequence length

	/* This maps the ADC channel number to the position in the conversion sequence */
	// IN3 IN10 IN16 IN17 IN11 IN12 for conversions sequences
	ADC1_SQR3 = ( (12<<25) |(11<<20) | (17<<15) | (16<< 10) |(10 << 5) |( 3 << 0)); 
	...
	NVICIPR (NVIC_I2C2_EV_IRQ, NVIC_I2C2_EV_IRQ_PRIORITY );
	NVICISER(NVIC_I2C2_EV_IRQ);		
	return;
}
/******************************************************************************
 * static void adcsensor_vcal_start_cal_register_reset(void);
...
/******************************************************************************
 * static unsigned int adcsensor_vcal_start_conversion(void);
 ...
 *####################################################################################### */
void DMA1CH1_IRQHandler_vcal(void)
{
...
'cnt' is a running counter of sequences converted.  Maybe not too useful except for debugging */
	...
	NVICISPR(NVIC_I2C2_EV_IRQ);	// Set pending (low priority) interrupt
//I2C2_EV_IRQHandler_yog();
...
void 	(*dma_ll_ptr)(void) = 0;		// DMA high priority ->  2C2_EV_IRQHandler_yog (low priority)

void I2C2_EV_IRQHandler_yog(void)
{
...
volatile int32_t	adc_temp_flag[NUMBERADCCHANNELS_TEN];	// Signal main new filtered reading ready
...
static void adc_cic_init_ycal(void)
{
	...
	dma_ll_ptr = &adc_cic_filtering_ycal;
 ...
 * ############################################################################################### */
/* 
   This routine is entered from  'FSMC_IRQHandler' via 'dma_ll_ptr' triggered by 'DMA1CH1_IRQHandler_ycal'
*/
