...
void p1_PC_can_monitor_msgs(void)
{
	...
	printf(" %04x %04x %08x %08x\n\r",(unsigned int)temp.id,(unsigned int)temp.dlc,(unsigned int)temp.cd.ui[0],(unsigned int)temp.cd.ui[1]);
...
//	char *pc;
	...
	{ // Here, there were no id's found
		USART2_txint_puts("No messages are being logged\n\r"); USART2_txint_send(); return;
	}
	printf ("%3u: ",idxLast);
	for (i = 0; i < idxLast; i++)
	{ // List'em all
