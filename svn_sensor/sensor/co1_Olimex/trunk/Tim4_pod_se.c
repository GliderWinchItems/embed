...
void setnextoc(void)
{
	...
	can_sync_msg.cd.ui[0] = strAlltime.SYS.ui[0];
	can_sync_msg.cd.ui[1] = strAlltime.SYS.ui[1];
	
	can_msg_put((struct CANRCVBUF *)&can_sync_msg);	// Set up CAN thyme (pronounced "time") sync msg

	can_log_puttime((struct CANRCVBUF *)&can_sync_msg, strAlltime.SYS.ull);	// Set up CAN time sync msg for logging

	while ( (p = gpsfix_canmsg_get_can()) != 0)	// Add any gps msgs, e.g. lat/lon and ht fixes
		can_msg_put(p);
...
void TIM4_IRQHandler(void)
{
	...
	/* Extended timing: Handle the 1 PPS input capture (CC1IF) and counter overflow (UIF) flags */
	switch (usSR & 0x3)
	{
