...
//					}
				...
				/* Convert the lat/long to binary, scaled, minutes * 1E5. */
				tmp = gps_getfix_GPRMC(&gpsfix, strlb);
				if ( tmp == 0)
				{ // Success.  Send gps fix on CAN bus, and log it. 
					// Add to buffer and send to CAN: ID, pointer to payload, payload count
					gpsfix_canmsg_add( (CAN_UNITID_CO_OLI | CAN_DATAID_LAT_LONG), (char*)&gpsfix.lat, 8);
				}
				else
				{ // Here, some error in the extraction and conversion
					sprintf (vv,"$GPRMC FIX ERROR: %u: %s\n\r",tmp, strlb.p);
...
{
	/* Set up id and our linux time stamp */
	gpdcanbuflog[gpscanbuflogIDXi].id = id	// Be sure we have it tagged with the ID
	gpdcanbuflog[gpscanbuflogIDXi].U.ull = strAlltime.SYS.ull;	// Add extended linux format time

	/* Set payload count */
	if (count > 8) count = 8; gpdcanbuflog[gpscanbuflogIDXi].dlc = count;
	
	/* Copy payload to buffer */
	gpdcanbuflog[gpscanbuflogIDXi].cd.ull = 0;	// Zero out unused payload bytes
	for (i = 0; i < count; i++)
		gpdcanbuflog[gpscanbuflogIDXi].cd.c[i] = *ppay++;
	
	/* Save a copy in the log buffer in the can buffer (that will go on the CAN bus). */
	gpscanbufcan[gpscanbufcanIDXi] = gpdcanbuflog[gpscanbuflogIDXi];
...
struct CANRCVSTAMPEDBUF* gpsfix_canmsg_get_log(void)
{
	...
	p = &gpdcanbuflog[gpscanbuflogIDXm]; 	// Set pointer
...
struct CANRCVBUF* gpsfix_canmsg_get_can(void)
{
	...
	pp = &gpscanbufcan[gpscanbufcanIDXm]; 	// Set pointer
