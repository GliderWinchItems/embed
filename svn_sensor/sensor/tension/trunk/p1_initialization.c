...
//	unsigned int ui;
	...
	ad7799_ten_rd_ID_reg (n);		// Read ID register (macro in ad7799_comm.h)
	while (ad7799_ten_comm_busy != 0);	// Wait for write/read sequence to complete
	idreg = ad7799_ten_8bit_reg;
	printf ("ID    :   %06X \n\r",idreg);

	/* Setup CH-2: Zero calibration, sampling rate, and continuous conversions */
	ad7799_ten_1_initA(AD7799_4p17SPS);	// Setup for strain gauge, (Arg = sampling rate)
	while (ad7799_ten_comm_busy != 0);	// Wait for write/read sequence to complete

	ad7799_ten_rd_configuration_reg(n);	// Read configuration register (macro in ad7799_comm.h)
	while (ad7799_ten_comm_busy != 0);	// Wait for write/read sequence to complete
	ui = ad7799_ten_16bit.us;
	printf ("Config: %08X \n\r",ui);

	ad7799_ten_rd_mode_reg(n);		// Read mode register (macro in ad7799_comm.h)
	while (ad7799_ten_comm_busy != 0);	// Wait for write/read sequence to complete
	ui = ad7799_ten_16bit.us;
	printf ("Mode  : %08X \n\r",ui);

	ad7799_ten_rd_offset_reg (n);		// Read offset register (macro in ad7799_comm.h)
	while (ad7799_ten_comm_busy != 0);	// Wait for write/read sequence to complete
	printf ("Offset: %8X \n\r",(int)ad7799_ten_24bit.n);
	ten_f[n].offset_reg = ad7799_ten_24bit.n;

	ad7799_ten_rd_fullscale_reg(n);		// Read full scale register (macro in ad7799_comm.h)
	while (ad7799_ten_comm_busy != 0);	// Wait for write/read sequence to complete
	printf ("FScale: %8X \n\r",(int)ad7799_ten_24bit.n);
	ten_f[n].fullscale_reg = ad7799_ten_24bit.n;
