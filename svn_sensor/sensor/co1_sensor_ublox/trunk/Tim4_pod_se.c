...
* Description        : Input capture with TIM4 CH3 (PB6) on POD board.
...
/*
TIM4 CH3 IC (input capture) is driven by the GPS 1 PPS.  This is used to phase the
...
volatile unsigned int tim4ticks;
int db0;
unsigned int db1;
unsigned int db2;
unsigned int db3;
...
/* CAN time sync msg */
struct CANRCVBUF can_sync_msg;

#define CANSYNCBUFSIZE	4
static struct CANRCVBUF can_sync_buf[CANSYNCBUFSIZE];
...
volatile union TIMCAPTURE64	strTim4cnt;	// 64 bit extended TIM4 CH3 timer count
// Input capture extended counts
volatile union TIMCAPTURE64	strTim4;	// 64 bit extended TIM4 CH3 capture
volatile union TIMCAPTURE64	strTim4m;	// 64 bit extended TIM4 CH3 capture (main)
...
/* TIM4 CH3: PB6 GPS 1 PPS, for alternate function input p 156, p 163 */
const struct PINCONFIGALL pb8  = {(volatile u32 *)GPIOB, 8, IN_FLT, 0};
 ...
 * @return	: NULL = buffer empty, otherwise pointer to msg
*******************************************************************************/
struct CANRCVBUF* Tim4_pod_se_sync_msg_get(void)
{
	struct CANRCVBUF* ptmp;
	if (canbufidxi == canbufidxm) return NULL;
	ptmp = &can_sync_buf[canbufidxm];
	canbufidxm += 1; if (canbufidxm >= CANSYNCBUFSIZE) canbufidxm = 0;
	return ptmp;
}
/******************************************************************************
...
void Tim4_pod_se_set_sync_msg(void)
{
	can_sync_msg.id = gps_f.gps_s.cid_timesync;	// CAN ID for time sync
	can_sync_msg.dlc = 1;	// One byte payload (tick within second count)
	return;
}
/******************************************************************************
...
void Tim4_pod_init(void)
{
	...
	/*  Setup TIM4 CH3: PB6 GPS 1 PPS, for alternate function input p 156, p 163  */
	pinconfig_all((struct PINCONFIGALL *)&pb8);	

	/* TIMx capture/compare mode register 1  (p 250 fig 125, and 399) */
	TIM4_CCMR2 |= 0x1;	// 01: CC3 channel is configured as input, IC3 is mapped on TI3
	...
	// CH3 Configured as input: rising edge trigger
	// CH2 Configured as output: active high
	TIM4_CCER |= (0x1 << 8) | (0x1 << 4); 	//  p 401
...
http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.ddi0337g/BABJFFGJ.html */
	...
	*(volatile unsigned int*)0xE0001000 |= 0x1;	// Enable DTW_CCNCNT (Data Watch counter)
	...
	NVICIPR (NVIC_TIM4_IRQ, TIM4_PRIORITY_CO );	// Set interrupt priority
	NVICISER(NVIC_TIM4_IRQ);			// Enable interrupt controller for TIM4
	
	/* Enable input capture interrupts */
	// Enable CH2 output compare, CH3 capture, and counter overflow interrupts (p 393)
	TIM4_DIER |= (TIM_DIER_CC3IE | TIM_DIER_CC2IE | TIM_DIER_UIE);	// All three
...
struct TIMCAPTRET32 Tim4_inputcapture_ui(void)
{
	...
	TIM4_DIER &= ~(TIM_DIER_CC3IE | TIM_DIER_UIE);	// Disable CH3 capture interrupt and counter overflow (p 315)
	tmp = TIM4_DIER;				// Readback ensures that interrupts have locked

	strY.ic  = strTim4m.ui[0];			// Get 32b input capture time
	strY.flg = uiTim4ch3_Flag; 			// Get flag counter
	TIM4_DIER |= (TIM_DIER_CC3IE | TIM_DIER_UIE);	// Enable CH3 capture interrupt and counter overflow (p 393)
...
//	tim4_oc_ctr = 0;	// Interval counter
...
tim4debug7 = tim4_oc_ctr;
	...
	{ // Here, the ending OC was after the 1 PPS, so longer OC interval times are needed
		...
		diff = ((int)TIM4_CCR3 - (int)tim4_ccr2);	// (1 PPS IC - OC)
...
tim4debug0 = diff;
	...
	else
	{
		...
		{ // Here, the OC ending has yet to occur, so shorter OC interval times are needed
			/* Compute the time difference */
			diff = ((int)TIM4_CCR2 - (int)TIM4_CCR3);	// (OC - 1 PPS IC)
...
tim4debug0 = -diff; // 
		...
		{ // Here, we are further out than one OC interval, so "jam" i.e re-initialize.
			TIM4_CCR2 = (TIM4_CCR3 + TIM4NOMINAL);	// Set a new "nominal" increment
...
void setnextoc(void)
{
	...
	{ // Here, the end of the last interval is the demarcation of the 1/64th sec tick
		...
		// Add msg to buffer for CAN_poll_loop.c to dispatch
		can_sync_buf[canbufidxi] = can_sync_msg;	// Copy to buffer
		canbufidxi += 1; if (canbufidxi >= CANSYNCBUFSIZE) canbufidxi = 0;
		CAN_poll_loop_trigger();	// Trigger CAN_poll_loop
...
//	TIM4_CCR2 += (TIM4NOMINAL + tmp);	// Set next interval end time
...
void TIM4_IRQHandler(void)
{
	...
	/* Extended timing: Handle the 1 PPS input capture (CC3IF) and counter overflow (UIF) flags */
	switch (usSR & 0x9)
	{
...
tim4debug4 = db4;
tim4debug6 = db6;
		strTim4.us[0] = TIM4_CCR3;		// Read the captured count which resets the capture flag
		...
		uiTim4ch3_Flag += 1;			// Advance the flag counter to signal mailine IC occurred
...
tim4debug6 = db6;
		...
		strTim4m = strTim4;			// Update buffered value		
		uiTim4ch3_Flag += 1;			// Advance the flag counter to signal mailine that an IC occurred
	...
	/* OC output duration & phasing: Handle the 1 PPS input capture (CC3IF) and CH2 output capture (CC2IF) flags */
	switch (usSR & 0xC)	
	...
	case 0x4:	// OC only
		...
		break;
	case 0xC:	// IC and OC were both on at isr entry (i.e. when usSR stored the SR)

			/* Is IC ahead of OC? */
			diff = ((int)TIM4_CCR3 - (int)TIM4_CCR2);	// IC - OC (registers)
			if (diff < 0)
				diff += 65536;
...
//	strIC = Tim4_inputcapture_ui();			// Get input capture time and flag ctr
	...
	strIC.flg = uiTim4ch3_Flag; 
	ticks_temp = (s32)(strIC.ic - Tim4tickctr_prev);// Ticks in one second
	Tim4tickctr_prev = strIC.ic;			// Save for computing difference next time

	ticks_flg += 1;	// main will use this for pacing
	ticks = ticks_temp;
