...
char * gps_time_find_dollars(struct USARTLB strlb )
{
	if (strlb.p == NULL) return 0;

	/* Search forward for a '$' */
	while ((*strlb.p != '$') && (*strlb.p != 0) && (strlb.ct-- > 0)) strlb.p++;
	
	if ((*strlb.p == 0) || (strlb.ct == 0) ) return 0;	// Return = not a valid sentence (no '$')
...
int gps_getfix_GPRMC(struct GPSFIX* p, struct USARTLB strlb)
{
	char* px = strlb.p;	// Convienent local pointer
	int mm;

	/* Search forward to the '$'.  If not present something bogus. */
	if ( (px=gps_time_find_dollars(strlb)) == 0) return -1; // return invalid
	
	p->ht = 0;	// No height with GPRMC

	/* Spin forward to Latitude field. */
	px = gps_field(px, 3, 64 );

	/* Extract degrees from latitude field and convert to binary. */
	// Expect input of 'ddmm.mmmm' (d = degs, m = minutes)
	p->lat  =  asctobin(*px++);
	p->lat  =  (p->lat * 10) + asctobin(*px++);
	p->lat *= 60;	// Convert degrees to minutes
	
	/* Extract minutes, convert to binary and scale to make whole number */
	mm      =  asctobin(*px++);
	mm     =  (mm * 10) + asctobin(*px++);
	p->lat += mm;
	if (*px++ != '.') return -2; 	// Skip '.'
	p->lat = (p->lat * 10 )+ asctobin(*px++);
	p->lat = (p->lat * 10 )+ asctobin(*px++);
	...
	p->lat = (p->lat * 10 )+ asctobin(*px++);
	p->lat = (p->lat * 10 )+ asctobin(*px++);
	p->lat = (p->lat * 10 ); // ublox has 5 digit minutes fraction

	/* Convert N-S to +/- */
	px++;	// Skip ',' at end of field
	if (*px == 'S')
		p->lat = -p->lat;
	else {if (*px != 'N') return -4;} // Must be either 'N' or 'S'	
	px++;
	
	if (*px++ != ','){ return -5;} // Skip ',' at end of field

	/* Extract degrees from latitude field and convert to binary. */
	// Expect input of 'dddmm.mmmm' (d = degs, m = minutes)
	p->lon  =  asctobin(*px++);
	p->lon  =  (p->lon * 10) + asctobin(*px++);
	p->lon  =  (p->lon * 10) + asctobin(*px++);
	p->lon *= 60;	// Convert degrees to minutes
	
	/* Extract minutes, convert to binary and scale to make whole number */
	mm = asctobin(*px++);
	mm = (mm * 10) + asctobin(*px++);
	p->lon += mm;
	if (*px++ != '.') return -6; 	// Skip '.' for fractional minutes
	p->lon = (p->lon * 10 )+ asctobin(*px++);
	p->lon = (p->lon * 10 )+ asctobin(*px++);
	p->lon = (p->lon * 10 )+ asctobin(*px++);
	p->lon = (p->lon * 10 )+ asctobin(*px++);
	p->lon = (p->lon * 10 )+ asctobin(*px++);
	p->lon = (p->lon * 10 ); // ublox has 5 digit minutes fraction

	/* Convert W-E to +/- */
	px++;	// Skip ','
	if (*px == 'E')
		p->lon = -p->lon;
	else {if (*px != 'W') return -7;} // Must be either 'W' or 'E'

	p->ht = 0;	// No height in GPRMC message
	return 0;		
 ...
 ******************************************************************************/
static const char *pPubx00 = "$PUBX,00,";	// Sentence header to match
int isGps_PUBX00(struct USARTLB strlb)
{
	int i;
	char* p = strlb.p;
	char * pp = (char*)pPubx00;
	/* Search forward to the '$'.  If not present something bogus. */
	if ( (p=gps_time_find_dollars(strlb)) == 0) return -1; // return invalid
...
int gps_getfix_PUBX00(struct GPSFIX* p, struct USARTLB strlb)
{
	...
	if (strlb.ct < 10) return -10;
	if (isGps_PUBX00(strlb) != 0) return -9;

	/* Search forward to the '$'.  If not present something bogus. */
	if ( (px=gps_time_find_dollars(strlb)) == 0) return -1; // return invalid
	
	/* Spin forward to type of FIX */
	ptmp =  gps_field(px, 8, 64 );
	if (*ptmp++ == 'N') return -12;	// Return: No Fix
	p->fix = asctobin(*ptmp); // Fix type
	
	/* Spin forward to number of satellites */
	ptmp =  gps_field(px, 18, 120 );
	p->nsats = asctobin(*ptmp++); // Number of satellites (units)
	if (*ptmp != ',') // A two digit field?
		p->nsats = (p->nsats * 10) + asctobin(*ptmp); // Number of satellites (tens)
	...
	/* Convert height */
	px++;	// Move to ','
	if (*px++ != ',') return -11;  // Return if '.' not found
	p->ht = 0;
	// Convert up to decimal pt
	while ((*px != '.') && (*px != ',') && (*px != '\n') )
	{
		p->ht = (p->ht * 10) + asctobin(*px++);
	}
	if (*px == ',') return -8;  // Return if '.' not found
	if (*px++ == '\n') return -8;  // Return if hit newline
	for (i = 0; i < 3; i++)  // Convert fraction
	{
		p->ht = (p->ht * 10) + asctobin(*px++);
	}
	return 0;
