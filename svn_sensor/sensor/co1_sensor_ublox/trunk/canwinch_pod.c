...
void can_nxp_setRS(int rs, int board)
{
	...
	if (board == 0)
	{
		...
		if (rs == 0)
			GPIO_BRR(GPIOD)  = (1<<11);	// Reset bit LO for SILENT mode
		else
	...
	else
	{
		...
		if (rs == 0)
			GPIO_BRR(GPIOB)  = (1<< 7);	// Reset bit LO for SILENT mode
		else
...
struct CANRCVSTAMPEDBUF* canrcv_get(void)
{
	...
	canbufIDXm = adv_index(canbufIDXm, CANRCVBUFSIZE); // Advance index
...
void USB_LP_CAN_RX0_IRQHandler(void)
{
	...
	canbuf[canbufIDXi].dlc	    = CAN_RDT0R(CAN1) & ~0x0000fff0;	// time, data length CAN_RDTxR p 663
...
//while(1==1);
	...
	cantimbuf[canbuftimIDXi].U.ull    = strAlltime.SYS.ull;	// Get current tick time
	cantimbuf[canbuftimIDXi].R.id     = CAN_RI1R(CAN1);	// ID, RTR, IDE
	cantimbuf[canbuftimIDXi].R.dlc	  = CAN_RDT1R(CAN1) & ~0x0000fff0;	// time, data length CAN_RDTxR p 663
	...
	if ( (cantimbuf[canbuftimIDXi].R.id & ~0x7) == ( CAN_RESETALL | CAN_DATAID_MASTERRESET ) )
	{
		SCB_AIRCR = (0x5FA << 16) | SCB_AIRCR_SYSRESETREQ;	// Cause a RESET
	}
	...
	/* Trigger a pending interrupt, which will cause a chain of related routines to execute */
	// This is the same one as for FIFO 0 above, and will end up going through the buffering for can_log.c.
	NVICISPR(NVIC_I2C1_ER_IRQ);	// Set pending (low priority) interrupt for TIM4 ('../lib/libusartstm32/nvicdirect.h')
