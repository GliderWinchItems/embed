...
uint32_t tim3_ten2_rate;

/* Reduce the rate for low level interrupt triggering. */
#define TIM3LLTHROTTLE	4 	// Trigger count
static unsigned int throttlect = 0;

/* Running count of timer ticks. */					
unsigned int tim3_ten2_ticks;	// Running count of timer ticks

/* Increment for CH1, 2 output capture interrupts. */
static uint16_t inc; 	// CH1 Timer tick increment for regular interrupts
static uint16_t vinc;	// Temporary CH2 increment
