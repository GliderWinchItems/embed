...
int CAN_poll_loop_init(void)
{
	...
	if (phub_tension_a == NULL) return -2;
//	tension_a_function_init();  ### This is done at main ###
#endif

#ifdef HUB_TENSION_B
 	phub_tension_b = can_hub_add_func();	// Get a hub port tension_b
	if (phub_tension_b == NULL) return -3;
//	tension_b_function_init();
#endif

#ifdef HUB_CABLE_ANGLE
 	phub_cable_angle1 = can_hub_add_func();	// Get a hub port cable angle
	if (phub_cable_angle1 == NULL) return -4;
