...
/* --------------------- CAN setup ------------------------------------------------------------------- */
	...
	pctl1 = canwinch_setup_F103_pod(140, 32, 8);
...
/* In the following TX uses 'xw' and RX uses 'xb[]' */
union CAN_X
{
	u32	xw;	// TX dtw
	u8	xb[4];	// RX 
};
#define retryct xb[0]		// Counter for number of retries for TERR errors
#define nosend	xb[1]		// Do not send: 0 = send; 1 = do NOT send on CAN bus (internal use only)
#define maxretryct xb[2]	// Maximum number of TERR retry counts
#define bits	xb[3]		// Use these bits to set some conditions (see below)

struct CAN_POOLBLOCKX	// Used for common CAN TX/RX linked lists
{
	...
	printf("sizeof (struct CAN_XMITBUFF) %d\n\r",sizeof (struct CAN_XMITBUFF)       );	USART1_txint_send(); 
	printf("sizeof (struct CANRCVBUF)    %d\n\r",sizeof (struct CANRCVBUF)    	);	USART1_txint_send(); 
	printf("sizeof (struct CANRCVTIMBUF) %d\n\r",sizeof (struct CANRCVTIMBUF) 	);	USART1_txint_send(); 
	printf("address of frii %08X\n\r",(unsigned int)&pctl1->ptx.frii                );   	USART1_txint_send(); 

	printf("sizeof (struct CAN_POOLBLOCK) %d\n\r",sizeof (struct CAN_POOLBLOCKX) 	);	USART1_txint_send(); 
...
//			can_driver_printerr(pcanerrors);USART1_txint_send();
		...
		pFifo1 = can_driver_get1(pctl1);	// Check for FIFO 1 msgs
		if (pFifo1 != 0)
		...
		pFifo0 = can_driver_get0(pctl1);	// Check for FIFO 0 msgs
		if (pFifo0 != 0)
		{ // Here, we got something!
			cmd_n_F103(pFifo0);
			printmsg("F0: ",pFifo0);
