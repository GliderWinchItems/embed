...
{	
	...
	if (fabsf(f) < (3.0503E-5) )
	{
		flt.f = 0;
	}

	uint32_t x = (flt.ui & ~0x80000000) + 0x00001000;
	flt.ui  = (flt.ui & 0x80000000) | x;
...
void floattopay3qtrfp(uint8_t *p, float f)
{
	union UIF flt;
	flt.f = f;
	*(p+0) = flt.ui >>  8; //flt.uc[1];
	*(p+1) = flt.ui >> 16; //flt.uc[2];
	*(p+2) = flt.ui >> 24; //flt.uc[3];
	return;
}
/******************************************************************************
...
float pay3qtrfptofloat(uint8_t *p)
{
	union UIF flt;
	flt.ui  = 0;		// flt.uc[0] = 0;
	flt.ui |= *(p+0) << 8; 	// flt.uc[1] = *(p+0);
	flt.ui |= *(p+1) << 16;	// flt.uc[2] = *(p+1);
	flt.ui |= *(p+2) << 24;	// flt.uc[3] = *(p+2);
	return flt.f;
}	
/******************************************************************************
...
{
	union UIF flt;
	flt.ui  = *(p+0) <<  0;	// flt.uc[1] = *(p+0);
	flt.ui |= *(p+1) <<  8;	// flt.uc[1] = *(p+1);
	flt.ui |= *(p+2) << 16;	// flt.uc[2] = *(p+2);
	flt.ui |= *(p+3) << 24;	// flt.uc[3] = *(p+3);
