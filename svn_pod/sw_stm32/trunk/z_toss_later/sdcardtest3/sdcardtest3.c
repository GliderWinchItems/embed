...
//	clocksetup();		// Get the system clock and bus clocks running
	...
	sdcard_csd_print_POD = 1;	// 0 = USART2; 1 = USART1
	sdcard_cid_print_POD = 1;	// 0 = USART2; 1 = USART1
	...
	*/
	...
	/* Announce who we are */
	USART1_txint_puts("\n\rsdcardtest2 USART1 txint rxint 09-21-2011\n\r");
...
//	sdcard_csd_list();
	...
	USART1_txint_puts("Enter: 'p xxxx<ret>' to read block number xxxx into buffer and display \n\r");	USART1_txint_send();
	USART1_txint_puts("Enter: 'w xxxx<ret>' to write block number xxxx out of buffer \n\r");		USART1_txint_send();
	USART1_txint_puts("Enter: 'c xxxx|yyyy <ret>' to compare block x with block y \n\r");			USART1_txint_send();
...
//			if (lb.ct > 0)				// Check if we have a completed line
		...
		{ // Here we have a pointer to the line and a char count
			...
			case 'p':
				...
				print_cawreal (caw_realdata); 	USART1_txint_send();	// Display block
				break;
			...
			case 'c':
				...
				for ( i = 0; i <SDC_DATA_SIZE; i++)
				{
					...
					{
						printf ("%3u %02x %02x\n\r",i,caw_realdata[i],caw_realdataX[i]);USART1_txint_send();
						l = 1;	// Set flag
					}
				}
				if (l == 0 )
