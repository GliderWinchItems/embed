...
unsigned int uiMax;	// Find max time
unsigned long long ulAve;	// Ave write time
/* Crude histogram of write times */
#define BINCT	16	// Number of bins
unsigned int uiHist[BINCT];	

/* Switch to write zeroes rather than random data for command 'v' */
...
/*****************************************************************************************/
/* Stupid routine for toggling the gpio pin for the LED -- Olimex */
/*****************************************************************************************/
...
//	clocksetup();		// Get the system clock and bus clocks running
	...
	SDCARDREG_on;		// See 'svn_pod/sw_stm32/devices/PODpinconfig.h' for macro

	systick1 = SYSTICK_getcount32() - (5000*(pclk1_freq/10000));	// Get 32 bit before starting card setup
	while ( ((int)(SYSTICK_getcount32()- systick1)) > 0);	// Wait for power supply 
	...
	*/
	...
	if ( (temp = USART_rxinttxint_init(115200,32,2,96,4)) != 0 )
	...
	/* Announce who we are */
	USART_txint_puts("\n\rsdcardtest4 USART_ txint rxint 09-22-2011\n\r");
...
//	printf ("spi2 divide code : %9u\n\r",spi2_sclk_get_divisor_code()); 	USART_txint_send();	
	...
	USART_txint_puts("Enter: 'z xxxx<ret>' clear write protect block xxxx\n\r");	USART_txint_send();
	USART_txint_puts("Enter: 'v xxxx<ret>' to write blocks 0 to (xxxx-1) to 0s\n\r");	USART_txint_send();
	USART_txint_puts("Enter: 'l xxxx<ret>' sdlog write xxxx records & test\n\r");	USART_txint_send();
...
//			if (lb.ct > 0)				// Check if we have a completed line
		...
		{ // Here we have a pointer to the line and a char count
			...
			/* Command 'v xxxx <ret>' writes from block 0 to (xxx-1) with all zeroes */
			case 'v':
				uiMax = 0; ulAve = 0; usZero = 1;
				NEXT_STATE(a_funct);
				data_mask = 0;
				caw_arg = dumbasctoint(lb.p + 1);
				caw_flag = (1==0);
...
void a_00(void)
{
	...
	sdc24_write_block(block_no, (unsigned char *)caw_realdata);
...
void a_01(void)
{
	...
	/* Build crude histogram */
	temp = (uiSysDur/(sysclk_freq/1000000)) / 10000;
	if (temp >= BINCT ) temp = BINCT-1;
	uiHist[temp] += 1;
	...
	if((block_no % bpp) == 0)
	{
		...
		{
			uiSysDur = ulAve/10000;
			printf("\r\n%u.w.%07u.%06u.%06u> ", pass_no, block_no, uiMax/(sysclk_freq/1000000),uiSysDur/(sysclk_freq/1000000) );
			ulAve = 0;uiMax = 0;

			for (temp = 0; temp < BINCT; temp++) printf("%7u",uiHist[temp]);
	
			/* Clear histogram counters */
			for (temp = 0; temp < BINCT; temp++) uiHist[temp] = 0;
		}
		USART_txint_send();
...
void b_00(void)
{
	sdc17_read_block(block_no, (unsigned char *)caw_realdata);
