...
The battery and thermistor readings are averaged.

The output rate of the accelerometer data is the same as the tension data (ad7799), i.e. 64 samples
per second, arrived at by a data input rate of 2048 and a decimation rate of 32.  The battery and
thermistor data is slowly changing so there is no need for a fast output rate.  Given the simplicty
of computing an average the data can be accumulated to a maximum of 2^20 which would give an output every
512 seconds.  Accumulation of 65536 readings (followed by a shift right of 16 bits) gives an average
with an ouput rate of once every 16 seconds, which is a reasonable rate for this data.
...
unsigned short	usAccelpktctr;		// Incremented each time a 
...
static unsigned short usPktIdx;		// Index for entries within packet
...
static unsigned short usMonIdx;		// Index for array buffer being emptied (monitor)
...
static short usDiscard = ACCELDISCARDNUMBER;	// Initial discard counter for accelerometer readings
...
static unsigned short	usBattmpmonctr;		// Incremented each time a packet is ready
...
[2] - Top of battery */
static int adc_average[NUMBERADCREADINGSPKT];	// Beware of 32b accumulation when changing averaging length
static int nAdcaveragectr;		// Count number accumulated

static int adc_average_monitor[NUMBERADCREADINGSPKT];	// Beware of 32b accumulation when changing averaging length
static int nAdcaveragectrmonitor;	// Count number accumulated for monitoring
...
void adc_packetize_init(void)
{
	...
	for (i = 0; i < NUMBERACCELREADINGSPKT; i++)	// Setup decimation counts for filters
	{
		...
		if (strCIC_A[0].usFlag > 0)	// Is a filtered/decimated output ready? (@2)
		{
			...
			{ // Here, yes.  Go set it up in a packet for logging
				...
				{ // Here, yes.  Add the RTC tick counter to the packet
					accel_pktbuf[usIntIdx].U.ull =  strAlltime.LNX.ull;	// Add extended linux format time
				...
				accel_pktbuf[usIntIdx].U.ull = strAlltime.LNX.ull;	// Setup time stamp
...
/* ------------------------- Battery & Thermistor readings -------------------------------- */
	...
	adc_average[2] += strADC1dr.in[0][2];	// Top cell of battery	
	...
	{ // Here, yes.  Scale to less than 16 bits and store as a short
		...
		batttemp_pktbuf.U.ull =  strAlltime.LNX.ull;	// Add extended linux format time
		...
		batttemp_pktbuf.adc[2] =  adc_average[2] >> (ADCAVERAGEORDER-ADCAVERAGESCALING);// Top cell of battery
		...
		usBattmppktctr += 1;			// Flag that new data is ready
	}
	...
	if (nAdcaveragectrmonitor >= (1<<ADCORDERINAVERAGEMONITOR))	// Accumulated enough 
	{
		...
		batttemp_monbuf.U.ull = strAlltime.LNX.ull;	// Add extended linux format time
...
struct PKT_PTR adc_packetize_get_battmp_monitor(void)
{
	...
	if (usBattmpmonctr == 0) return pp; // Return no.
	usBattmpmonctr = 0;
