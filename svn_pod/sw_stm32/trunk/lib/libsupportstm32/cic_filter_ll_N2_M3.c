/******************************************************************************
* File Name          : cic_filter_ll_N2_M3.c
...
/******************************************************************************
 * unsigned short cic_filter_ll_N2_M3 (struct CICLLN2M3 *strX);
...
long long	llX1,llX2;	// Intermediate differentiator value
	...
	strX->llIntegral[0] += strX->nIn;		// Incoming data is 32 bits; add to 64 bits accumulator
	strX->llIntegral[1] += strX->llIntegral[0];		// 1st stage feeds 2nd stage
	strX->llIntegral[2] += strX->llIntegral[1];		// 2nd stage feeds 3rd stage
	
	/* Decimate (down-sample) */
	strX->usDecimateCt += 1;			// Decimation count
	if (strX->usDecimateCt >= strX->usDecimateNum) 	// Time to decimate?
	{ // Here, yes.  Do three stages of differentiation of the down sampled data
		strX->usDecimateCt = 0;		// Reset decimation counter
		llX1	           = strX->llIntegral[2] - strX->llDiff[0][1]; // 3rd stage integral minus 3rd stage value delayed by 2
		strX->llDiff[0][1] = strX->llDiff[0][0];	// Move the samples down one step
		strX->llDiff[0][0] = strX->llIntegral[2];	// Input goes into 1st delay of the two delays

		/* Repeat differentiator for 2nd stage */
		llX2               = llX1 - strX->llDiff[1][1]; // 1st stage diff- value delayed by 3
		strX->llDiff[1][1] = strX->llDiff[1][0];
		strX->llDiff[1][0] = llX1;

		/* Repeat for 3rd stage.  Output is the filtered/decimated output */
		strX->llout        = llX2  - strX->llDiff[2][1];
		strX->llDiff[2][1] = strX->llDiff[2][0];
		strX->llDiff[2][0] = llX2;
