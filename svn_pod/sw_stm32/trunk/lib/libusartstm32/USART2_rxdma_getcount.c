/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART2_rxdma_getcount.c 
* Hackor             : deh
* Date First Issued  : 10/10/2010 deh
* Description        : Get char for USART2 receive with no interrupts, using DMA and circular buffer
*******************************************************************************/ 
...
u16 USART2_rxdma_getcount(void)
{
	...
	int Diff = (int)(pUSARTcbr2->prx_end - (u32)DMA1_CNDTR6 - pUSARTcbr2->prx_now_i);
	if (Diff >= 0) 				// Is there a wrap-around?
		return Diff;			// Here, no wrap-around, or zero chars
	// Here there is a wraparound
	return (Diff + pUSARTcbr2->rx_ln_sz);  // Adjust for wrap
}
