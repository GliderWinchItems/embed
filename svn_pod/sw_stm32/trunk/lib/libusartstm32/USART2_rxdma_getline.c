...
* Hackor             : deh
* Date First Issued  : 10/10/2010 deh
...
{
	char c;					// Temp char
	u16 Count = USART2_rxdma_getcount();	// Count of chars ready in buffer
	...
	{ // Here, there are 1 or more chars in buffer
		c = USART2_rxdma_getchar();	// Save the char
		*pUSARTcbr2->prx_begin_m++ = c;	// Store the char in the line buffer
		if (pUSARTcbr2->prx_begin_m >= (char*)pUSARTcbr2->prx_ctary_now_i) // At end of buffer?
			pUSARTcbr2->prx_begin_m--;	// Prevent line buffer overflow
		if (c == END_OF_LINE)		// Test the char: Are we at an end of line?
		{ // Here, yes
			*pUSARTcbr2->prx_begin_m = 0; 	// Zero terminate the line
		         pUSARTcbr2->prx_begin_m = pUSARTcbr2->prx_begin_i; // Reset pointer to beginning
			return pUSARTcbr2->prx_begin_m;	// Return pointer to beginning of line (non-zero return)
		}
	}
