...
{
	u16 temp;
	if ( (temp = usartx_rxint_allocatebuffers(RcvLineBufferSize, RcvNumberLineBuffers, &pUSARTcbr2)) != 0) return temp;
	...
	usartx_rxint_usart_init (USART2, BaudRate);
...
{
	/* Receive */
	if (USART_SR(USART2) & USART_FLAG_RXNE)	// Receive register loaded?
