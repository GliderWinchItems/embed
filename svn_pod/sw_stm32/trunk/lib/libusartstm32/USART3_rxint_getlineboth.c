/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART3_rxint_getlineboth.c 
* Hackor             : deh
...
*/
	...
	if (pUSARTcbr3->prx_begin_m == pUSARTcbr3->prx_begin_i)
	{	
		return lb;			// Return zero, meaning nothing to do.
	}
	lb.ct = *pUSARTcbr3->prx_ctary_now_m;	// Return count in current buffer
	lb.p = usartx_rxint_rxmainadvptr (pUSARTcbr3); // Return pointer and step to next buffer
	return lb; 
}
