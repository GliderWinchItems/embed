...
//	{
//
//	 	*pUSARTcbt4->ptx_now++ = *p++;	// Store a char
//		if (pUSARTcbt4->ptx_now >= (pUSARTcbt4->ptx_begin_m + UART4_BUFFER_TX_LINE_SIZE) ) 
//		{
...
// wrestle with the problem of what to do.
	do 
	{
		...
		while (*(volatile char*)pUSARTcbt4->ptx_ctary_now_m != 0);	// Loop until a buffer is free

		/* Move the string into the buffer quickly with this assembly program */
		p = usartx_putsS(p,&pUSARTcbt4->ptx_now_m,(pUSARTcbt4->ptx_begin_m + pUSARTcbt4->tx_ln_sz) );
