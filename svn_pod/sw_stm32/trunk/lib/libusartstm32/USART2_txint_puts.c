...
//	{
//
//	 	*pUSARTcbt2->ptx_now++ = *p++;	// Store a char
//		if (pUSARTcbt2->ptx_now >= (pUSARTcbt2->ptx_begin_m + USART2_BUFFER_TX_LINE_SIZE) ) 
//		{
...
// wrestle with the problem of what to do.
	do 
	{
		...
		while (*(volatile char*)pUSARTcbt2->ptx_ctary_now_m != 0);	// Loop until a buffer is free

		/* Move the string into the buffer quickly with this assembly program */
		p = usartx_putsS(p,&pUSARTcbt2->ptx_now_m,(pUSARTcbt2->ptx_begin_m + pUSARTcbt2->tx_ln_sz) );
