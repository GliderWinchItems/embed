...
char USART3_txint_putc(char c)
{
	...
	while (USART3_txint_busy() == 1);	// Loop until buffer free

 	*pUSARTcbt3->ptx_now_m++ = c;	// Store a char

	/* Are we at the end of the buffer? */
	if (pUSARTcbt3->ptx_now_m >= (pUSARTcbt3->ptx_begin_m + pUSARTcbt3->tx_ln_sz) ) 
	{ // Here we hit the end of the line buffer, so 
		USART3_txint_send();	// Step to next line buffer
