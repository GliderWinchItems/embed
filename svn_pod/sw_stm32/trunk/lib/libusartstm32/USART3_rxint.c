/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART3_rxint.c 
* Hackor             : deh
* Date First Issued  : 10/12/2010 deh
* Description        : USART3 receive with char-by-char interrupts into line buffers
*******************************************************************************/ 
...
{
	u16 temp;
	if ( (temp = usartx_rxint_allocatebuffers(RcvLineBufferSize, RcvNumberLineBuffers, &pUSARTcbr3)) != 0) return temp;
	...
	usartx_rxint_usart_init (USART3, BaudRate);
...
{
	/* Receive */
	if (USART_SR(USART3) & USART_FLAG_RXNE)	// Receive register loaded?
