/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART2_rxint_getlineboth.c 
* Hackor             : deh
...
*/
	...
	if (pUSARTcbr2->prx_begin_m == pUSARTcbr2->prx_begin_i)
	{	
		return lb;			// Return zero, meaning nothing to do.
	}
	lb.ct = *pUSARTcbr2->prx_ctary_now_m;	// Return count in current buffer
	lb.p = usartx_rxint_rxmainadvptr (pUSARTcbr2); // Return pointer and step to next buffer
	return lb; 
}
