...
char UART4_txint_putc(char c)
{
	...
	while (UART4_txint_busy() == 1);	// Loop until buffer free

 	*pUSARTcbt4->ptx_now_m++ = c;	// Store a char

	/* Are we at the end of the buffer? */
	if (pUSARTcbt4->ptx_now_m >= (pUSARTcbt4->ptx_begin_m + pUSARTcbt4->tx_ln_sz) ) 
	{ // Here we hit the end of the line buffer, so 
		UART4_txint_send();	// Step to next line buffer
