/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART3_rxdma.c 
* Hackor             : deh
* Date First Issued  : 10/11/2010 deh
* Description        : USART3 receive with no interrupts, using DMA and circular buffer
*******************************************************************************/ 
...
struct USARTCBR* pUSARTcbr3;


/*******************************************************************************
 * u16 USART3_rxdma_init (u32 BaudRate,u16 RcvCircularSize, u16 GetLineSize);
 * @brief	: Initializes the USART3 to 8N1 and the baudrate for DMA circular
 *                buffering.
...
{
	u16 temp;
	if ( (temp = usartx_rxdma_allocatebuffers(RcvCircularSize, GetLineSize, &pUSARTcbr3)) != 0) return temp;
	...
	usartx_rxdma_usart_init (USART3, BaudRate);

	/* Setup DMA for usart receive, circular buffering, no interrupts */
	DMA1_CNDTR3 = RcvCircularSize;			// Number of incoming chars before wrap-around
	DMA1_CMAR3 = (u32)pUSARTcbr3->prx_begin;	// DMA chan 3 memory rx circular buffer start address
	DMA1_CPAR3 = (u32)&USART3_DR;			// DMA chan 3 periperal usart rx address
	DMA_CCR3(DMA1) = DMA_CCR3_MINC | DMA_CCR3_CIRC | DMA_CCR3_EN;   // Chan 3 Rx Mem increment, Circular, Enable
