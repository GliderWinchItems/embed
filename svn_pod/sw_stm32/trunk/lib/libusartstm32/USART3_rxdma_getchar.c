/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART3_rxdma_getchar.c
* Hackor             : deh
...
//	while (USART3_rxdma_getcount() == 0);	// Loop until something comes available
	/* Now we can get a char */
	c = *pUSARTcbr3->prx_now_i++;		// Store char, advance pointer	
	if (pUSARTcbr3->prx_now_i >= pUSARTcbr3->prx_end)	// Wrap around?
		pUSARTcbr3->prx_now_i = pUSARTcbr3->prx_begin;	// Reset pointer to beginning
	return c;
}
