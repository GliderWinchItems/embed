/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : USART1_rxdma_getchar.c
* Hackor             : deh
* Date First Issued  : 10/11/2010
...
//	while (USART1_rxdma_getcount() == 0);	// Loop until something comes available
	/* Now we can get a char */
	c = *pUSARTcbr1->prx_now_i++;		// Store char, advance pointer	
	if (pUSARTcbr1->prx_now_i >= pUSARTcbr1->prx_end)	// Wrap around?
		pUSARTcbr1->prx_now_i = pUSARTcbr1->prx_begin;	// Reset pointer to beginning
	return c;
}
