/******************** (C) COPYRIGHT 2011 **************************************
* File Name          : UART5_rxint_getlineboth.c 
* Hackor             : deh
...
*/
	...
	if (pUSARTcbr5->prx_begin_m == pUSARTcbr5->prx_begin_i)
	{	
		return lb;			// Return zero, meaning nothing to do.
	}
	lb.ct = *pUSARTcbr5->prx_ctary_now_m;	// Return count in current buffer
	lb.p = usartx_rxint_rxmainadvptr (pUSARTcbr5); // Return pointer and step to next buffer
	return lb; 
}
