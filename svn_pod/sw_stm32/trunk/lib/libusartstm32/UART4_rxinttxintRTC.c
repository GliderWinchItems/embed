/******************** (C) COPYRIGHT 2010 **************************************
* File Name          : UART4_rxinttxintRTC.c
* Hackor             : deh
* Date First Issued  : 07/14/2010 deh
...
struct USARTCBR* pUSARTcbr4;	// Receive
struct USARTCBT* pUSARTcbt4;	// Transmit
...
u16 UART4_rxinttxint_initRTC (u32 BaudRate,u16 RcvLineSize, u16 NumberRcvLines, u16 XmtLineSize, u16 NumberXmtLines)
{
	...
	/* Setup GPIO pin for GPIO_UART4 tx (PC10) (See Ref manual, page 158) */
	GPIO_CRH(GPIOC) &= ~((0x000f ) << (4*2));	// Clear CNF reset bit 01 = Floating input (reset state)
	...
	if ( (temp = usartx_rxint_allocatebuffers(RcvLineSize,NumberRcvLines,&pUSARTcbr4)) != 0) return temp;
	/* 'txdma' uses the same tx buffer setup as 'txint' */
	if ( (temp = usartx_txdma_allocatebuffers(XmtLineSize,NumberXmtLines,&pUSARTcbt4)) != 0) return temp+2;

	/* Set up usart and baudrate */
	/* 'txcir' sets up the usart Tx the same as needed here */
	usartx_txint_usart_init (UART4,BaudRate);// Tx enable
	/* 'rxint' of course is the same except for the interrup routine */
	usartx_rxint_usart_init (UART4,BaudRate);// Rx enable and enable interrupt

	/* Set and enable interrupt controller USART */
	NVICIPR(NVIC_UART4_IRQ, USART_PRIORITY );	// Set interrupt priority
	NVICISER(NVIC_UART4_IRQ);			// Enable interrupt	
	return 0 ;	// Success	
}
/*#######################################################################################
 * ISR routine
...
{
	/* Receive */
	if ( MEM_ADDR(BITBAND(UART4SR,RXFLAG)) != 0)	// Receive register loaded?
	{  // Here, receive interrupt flag is on. 
		*pUSARTcbr4->prx_now_i = USART_DR(UART4);	// Read and store char

		/* Advance pointers to line buffers and array of counts and reset when end reached */	
		usartx_rxint_rxisrptradvRTC(pUSARTcbr4);	// Advance pointers common routine
	}

	/* Transmit */
	if (MEM_ADDR(BITBAND(UART4CR1,TXFLAG)) != 0)	// Are Tx interrupts enabled?
	{  // Here, yes.  Transmit interrupts are enabled so check if a tx interrupt
		if (MEM_ADDR(BITBAND(UART4SR,TXFLAG)) != 0)	// Transmit register empty?
		{ // Here, yes.
			USART_DR(UART4) = *pUSARTcbt4->ptx_now_d++;	// Send next char, step pointer
			*pUSARTcbt4->ptx_ctary_now_d -= 1;	// Count down number to send in the line buffer
			if (*pUSARTcbt4->ptx_ctary_now_d == 0) 
			{ // Here, last char to be sent from this line buffer has been loaded into the tx register
				...
				usartx_txisr_advlnptr(pUSARTcbt4);	// Advance pointer for interrupt pointer ('_d)
				/* Are we caught up?  Check beginning of line buffer pointers, mainline versus interrupt  */	
//				if (pUSARTcbt4->ptx_begin_d == pUSARTcbt4->ptx_begin_m) // Interrupt versus Mainline pointers
				if (*pUSARTcbt4->ptx_ctary_now_d == 0)	// Is new line buffer ready?
				{ // Here, we are caught up with the main (where chars are being added)
					MEM_ADDR(BITBAND(UART4CR1,TXFLAG)) = 0x00;	// Disable Tx interrupts
				}
