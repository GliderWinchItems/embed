/******************** (C) COPYRIGHT 2011 **************************************
* File Name          : gpstest2.c
* Hackeroo           : deh
* Date First Issued  : 08/09/2011
* Board              : STM32F103VxT6_pod_mm
* Description        : Test program for 32 KHz freq calibration versus GPS
*******************************************************************************/

/* NOTE: 
1.  Open minicom on the PC with 115200 baud and 8N1 and this routine

2.  This is a hack of gpstest.c on 08/09/2011.  It uses 32KHz stuff for delays
...
#include "1_pps_cal_filter.h"	// Filter the time differences between 1_PPS & RTC
#include "adcppm_cal.h"		// Conversion of thermistor adc reading to 32 KHz osc error (ppm *100).
...
struct PPSCALFILT str1ppsIC;	// Filter time differences between Input Captures
struct PPSCALFILT str1ppsTH;	// Filter Thermister readings
...
char cSw;	// One time switch
int nICoffset;	// One-time Adjust IC1 v IC2 diff
int nIC12diffPrev;	// Difference in positions of IC1 & IC2, previous
int nIC12diff;		// Difference in positions of IC1 & IC2
int nIC12sum;
int nIC12ctr;
int nIC12x;
unsigned int uiIC2diffsum;
...
/* Conversion of ADC readings of thermistor */
unsigned int uiThermtmp;
unsigned int uiAdcTherm;
unsigned int uiThermtmpF;
unsigned int uiAdcppm;
unsigned int uiAdcppm2;

int nCtr;	// Count output lines
...
/* Make a 1/2 second tick flag */
volatile unsigned short usTick;		// Counter of RTCCLK (32768 Hz)divided by prescalar (PRL)
volatile unsigned char ucTflag;		// Flag to mainline for 1/2 sec tick

/* Accumulate 1/2 second's worth of ADC readings for the thermistor */
unsigned int uiAdcTherm;	// Accum for Thermister ADC readings
unsigned int uiAdcTherm_m;	// Accum for Thermister ADC readings for main

void RTC_secf_stuff(void)
{
	uiAdcTherm += strADC1dr.in[0][0];
	usTick += 1;		// 32768 Hz divided by 16
	if (usTick > 1024)	// Count 1/2 seconds worth then toggle
	{
		usTick = 0;	
		ucTflag += 1;	// Signal mainline program
		uiAdcTherm_m = uiAdcTherm;
		uiAdcTherm = 0;
	}
...
/* --------------------- ADC measurement startup ---------------------------------------------------- */
	...
	while ( ((int)((uiADCt0b = SYSTICK_getcount32()) - uiADCt0)) > 0 );	// Wait for regulator voltage to settle
	uiADCt1 = adc_init((volatile struct ADCDR*)&strADC1dr);	// Initialize the ADC for DMA reading of all POD adc pins
	while ( ((int)((uiADCt1b = SYSTICK_getcount32()) - uiADCt1)) > 0 );	// Wait for ADC to power up (1st write of CR2_ADON bit)
	uiADCt2 = adc_start_cal_register_reset();	// Start reset of calibration registers
	uiADCt3 = adc_start_calibration();		// Start calibration 
	while ( ((int)(SYSTICK_getcount32() - uiADCt4)) > 0 );	// Wait for battery voltage divider capacitors to charge
	uiADCt3 = adc_start_calibration();		// Start calibration 
...
//			printf ("%9u %9u  %9u ", uiGPSrtc, uiIC2, uiIC2-uiGPSrtc ); USART1_txint_send();
...
//			USART1_txint_puts("#\n\r");	USART1_txint_send();	// Show that we have valid time
		}
...
//				uiIC1ctr = 0;				// Reset flag counter
				...
				toggle_PA0_led();			// External LED in "BOX"
				

				nIC12diff = uiIC1diff-uiIC2diff;		// Raw difference between the IC's differences

				/* Accumulate for a 10 sec average */
				uiIC2diffsum += uiIC2diff;
				nIC12sum += nIC12diff; nIC12ctr += 1;

				/* Ten second display */
				if (nIC12ctr >= 10)
				{
					/* Line counter for reference later */
					printf("%4d ",nCtr++);

					/* Show average processor xtal (bus freq) */
					printf("%6d.%01u ",uiIC2diffsum/10, uiIC2diffsum - ((uiIC2diffsum/10)*10) );
					uiIC2diffsum = 0;

					/* Difference in RTC and GPS ticks between IC's */
					printf("%6d.%01u ",nIC12sum/10, nIC12sum - ((nIC12sum/10)*10) );
					nIC12x = (nIC12sum*10000)/24026;	// Adjust for 8 MHz err
					printf(" %6d.%02u ppm",(nIC12x/100), nIC12x - ((nIC12x/100)*100) );// Diff in ppm

					/* Average thermistor ADC reading (xxxxxx.x) */
					printf(" %6u.%01u adc ",uiAdcTherm_m/1024,( (uiAdcTherm_m*10)/1024) -( ( ( (uiAdcTherm_m*10)/1024)/10)*10) );
					...
					/* Display the ppm temperature adjustment */
					uiAdcppm2 = adcppm_cal(uiAdcTherm,strDefaultCalib.adcppm );	// Convert ADC reading to freq error (ppm * 100)
					output_100_scale(uiAdcppm2);		// Output as XXXX.XX
					USART1_txint_puts(" ppm  ");	
					output_100_scale(nIC12x-uiAdcppm2-strDefaultCalib.ppmoff);		// Output as XXXX.XX
					USART1_txint_puts(" ppm er\n\r");	
					

					USART1_txint_send();
					nIC12ctr = 0; nIC12sum = 0;
				}

//				if (pps_cal_filter(&str1ppsIC,nIC12diff) == 1)
//				{
//					printf ("                %9d \n\r",str1ppsIC.l1PPS_out>>15); USART1_txint_send();
//				}
...
//				printf (" %4u %4u ",uiIC1ctr, uiIC2ctr);// Flag counters 
//				printf ("T1: %10u %9u \n\r",uiIC1, uiIC1diff);
//				printf ("%9d ",nIC12diff);		// Difference in differences between RTC & 1_PPS
//				printf ("%10d ", nIC12diff - uiIC12diffPrev);
				nIC12diffPrev = nIC12diff;

				/* Display the thermistor temperature */
//				uiAdcTherm = uiAdcTherm_m/1024;		// Integer portion of ADC thermistor 1/2 sec average
//				uiThermtmp = adctherm_cal(uiAdcTherm);	// Convert ADC reading to temp (deg C)
//				output_100_scale(uiThermtmp);		// Output as XXXX.XX
//				USART1_txint_puts(" C");	
//				uiThermtmpF = (uiThermtmp*9)/5 + 3200;	// Convert to Farenheit
//				output_100_scale(uiThermtmpF);		// Output as XXXX.XX
//				USART1_txint_puts(" F");	

				/* Display the ppm temperature adjustment */
				uiAdcppm = adcppm_cal(uiAdcTherm,strDefaultCalib.adcppm );	// Convert ADC reading to freq error (ppm * 100)
//				output_100_scale(uiAdcppm);		// Output as XXXX.XX
//				USART1_txint_puts(" ppm");	
...
//				USART1_txint_puts ("\n\r");
		...
		{ // Here we have a pointer to the line and a char count
			if (*strlb.p == 's') break;		// STANDBY when s is the first char of the line
			if (*strlb.p == 'h') while(1==1);	// Hang
			if (*strlb.p == 'b')			// Battery readings display
			{
				USART1_txint_puts ("\r\n");	USART1_txint_send();
...
/*****************************************************************************************
Setup for output an unsigned with a 100* scale
*****************************************************************************************/
void output_100_scale(int uiS)
{
	if (uiS >= 0)
	{
		printf ("%4u.%02u",uiS/100,(uiS-((uiS/100)*100)) );
	}
	else
	{
		uiS = - uiS;
		printf ("-%3u.%02u",uiS/100,(uiS-((uiS/100)*100)) );	
	}
