...
h_hor = sqrt(pos_wst.^2 + pos_nrt.^2);	% horizontal distance
...
plot(pos_int - h_hor, h - wnch_h, 'r', 'linewidth', 2)
hold on
plot(pos_int - h_hor, f - f_zero, 'g', 'linewidth', 2)
plot(pos_int - h_hor, vec_v * 10, 'b', 'linewidth', 2)
title(banner)
ylim([-10 800])
grid on
xlabel('horizonatl dist (m)')
ylabel('height (m) tension (kgf) Vector(m/s * 10)')
legend ('Height', 'Tension','Vector vel' )
zoom on
title(banner)
