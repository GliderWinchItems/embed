/******************** (C) COPYRIGHT 2012 **************************************
* File Name          : xbloopbk_w
* Person             : deh
* Date First Issued  : 03/17/2012
* Board              : STM32F103VxT6_pod_mm
* Description        : Test program to loopbk two XBee units WINCH END of link
*******************************************************************************/
/*
03-27-2012 -- updated to use txdma and rxdma for USART2
*/
/* 
...
*/
	...
	/* Announce who we are */
	USART1_txint_puts("\n\rxbloopbk_w XBee loop back WINCH-END 03-27-2012 \n\r");
	...
	if ( (nCode=USART2_txdma_init(  9600,64,2) ) != 0) 	// Initialize USART and setup control blocks and pointers
	{
...
/* 
NOTE: USART2_txdma_puts(char* p) does a "send".  If USART2_txdma_send() follows it can hang waiting for a dma
completion that will never occur.
*/
	...
	xb_send_AT_cmd("ATRE,DL 2,MY 1,DL,MY,BD 7,CN\r");	// Library routine to do 'send_xb_cmd'
	...
	USART1_txint_puts("Change baud\n\r");	USART1_txint_send();
	USART2_setbaud(115200);
/* -------------------- Send/receive loop -------------------------------------------------------- */
	...
	while (1==1)
	{
		...
		{ // Here, pointer points to line buffer with a zero terminated line
			...
			/* Send line out USART2* */
			USART2_txdma_puts(p);		// Set up line to XBee
		}
