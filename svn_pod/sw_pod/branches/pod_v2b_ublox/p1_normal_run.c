...
{
	MAX3232SW_on
	sys.t1 = SYSTICK_getcount32() - (1000*(sysclk_freq/10000));
	while ( ( (int)(SYSTICK_getcount32() - sys.t1) ) > 0 );
...
//sys.t1 = SYSTICK_getcount32() - (5000*(sysclk_freq/10000));
//while ( ( (int)(SYSTICK_getcount32() - sys.t1) ) > 0 );

	p1_launchtime_init();		// Locate position in SD card for launch time data
	p1_pushbuttontime_init();	// Locate position in SD card for pushbutton time data
...
if ( uiDebug_normC++ > 100000) {uiDebug_normX = 0; uiDebug_normC = 0;}
		...
		/* Detect end of launch by monitoring tension */
		p1_launchtime_detect();

		/* Don't output readings until GPS time is acquired and settled down (a one-time setup that assures
		   the time correct before packets get written) */
		if (cGPS_ready != 0) 	// (@5)
		{ // Here, ready to log.
		...
		{ // Here, poll so that pushbutton is recognized.  NOTE: the 5 button pushes shutdown is detected here.
			pushbutton_packetize_poll();
		}
