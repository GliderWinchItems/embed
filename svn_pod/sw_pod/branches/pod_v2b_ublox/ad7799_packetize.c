...
static unsigned short usLaunchIdx;	// Index for packet being emptied (launch time detection)
...
static struct PKT_PTR ad7799_packetize_get_x(unsigned short *pusIdx)
{
	...
	*pusIdx = adv_index(*pusIdx, PKT_AD7799_BUF_CT);// Advance index for main
...
/******************************************************************************
 * struct PKT_PTR ad7799_packetize_get_launchtime(void);
 * @brief	: Get pointer & count to the buffer to be drained.
 * @return	: struct with pointer to buffer & sizeof, ptr & ct = zero if no new data
 ******************************************************************************/
struct PKT_PTR ad7799_packetize_get_launchtime(void)
{
	return	ad7799_packetize_get_x(&usLaunchIdx);		
}
