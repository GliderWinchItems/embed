...
static char cUpDn = 0;	// Going up = 0; down = 1;
...
static char packet_PC_buf[3*PACKETREADBUFSIZE];	// Hold ASCII output for PC (size defined in p1_common.h (@2))
...
Debugreadout += temp;
		...
		/* The return to this subroutine calls is either 2 or 3 and sets the next 'state'.  2 = OK, 3 = error */
		switch (temp)
		{
	...
	case 2: /* Output packet in hex ascii */
		...
		// If command was 'k' then the process is PC interaction to readout launchtimes
		if (state_PC_handler == 'k')
		{
			p1_PC_monitor_readout_launchtimes();
			state = 0;	// Back to beginning
			break;
		}
		// If command was 'q' then the process is PC interaction to readout launchtimes
		if (state_PC_handler == 'q')
		{
			p1_PC_monitor_readout_pushbuttontimes();
			state = 0;	// Back to beginning
			break;
		}
...
//						if  (ulpackettime > 1000)
			...
			case 3: // Here terminate readout because we hit the stop time
				...
				USART1_txint_puts((char *)stopmsg); // "< ##### Stop time has been reached at"
...
static int packet_setup_ad7799(struct PKT_AD7799 *pkt_ad7799, int temp, int id)
{
	...
	*pout++ = id+'0';			// Convert ID byte to ASCII
	...
	{
		ntemp = converttograms(pkt_ad7799->tension[i]);	// Convert to grams for ease of use
	/*-------------------------------------------------------------------------------------------*/
	/* This section is for detecting the beginning of launch during a launchtime readout. 
           See 'void p1_PC_monitor_readout_launchtimes(void)' further down in this mess. */
			switch (cUpDn)
			{
			case 0:  // Checking for tension going up
				if (ntemp <= LAUNCHTIMEREADOUT_TENSION_UP) // Have we exceeded the threshold?
					break;	// Here, no.  Keep on trucking
				// Here, release phase of launch detected
				cUpDn = 1;	// Tension is now over the high threshold
				break;		// Now in the real stuff of the launch

			case 1:	// Here looking for tension to drop to a low value signifying the beginning of launch
				if (ntemp >= LAUNCHTIMEREADOUT_TENSION_DN) // Has the tension dropped back down?
					break;	// Here, no. Still in the active launch phase
				// Here, yes.  We are at the beginning-of-launch detected
				cUpDn = 2;
				delayctr = 0;	// Add a few seconds of data at the beginning
				break;

			case 2: // Here we are adding data at the very beginning
				delayctr += 1;	// Count tension packets (0.25 secs per packet, 16 readings per packet)
				if (delayctr <= LAUNCHTIME_DELAY * PKT_AD7799_TENSION_SIZE) break;
				cUpDn = 3;	// Signal the cutoff of the readout
				break;

			case 3:	// By-pass this until someone resets 'cUpDn'. See 'void p1_PC_monitor_readout_launchtimes(void)' below
				break;
			}
	/*-------------------------------------------------------------------------------------------*/
...
static int launchtime_sub_cmd = 0;
static int launch_local_state = 0;
static int launchtime_yyyy;	// Offset number (from PC) into table of launchtimes
static unsigned int launchtime_limit;


void p1_PC_monitor_readout_launchtimes(void)
{
	unsigned int uitemp;
	struct LAUNCHTIME ltY;
	struct LAUNCHTIME ltX;

/* Dispatch is based on 'k' command's sub-command
Format
k x yyyy 
k = command code on menue
x = sub command
yyyy Count

Sub command x--
1 = return entry time for count yyyy, e.g. 0 = time of lastest end-of-launch, 1 = next one prior to current,...
2 = do readout of data for entry yyyy
 */

	switch (launchtime_sub_cmd)
	{
	case '1': // Command (ascii '1') = get entry (-yyyy), where yyyy is 0 - n, (going backwards in time)
		ltY = p1_launchtime_get_entry(launchtime_yyyy);
		/* Linux format time.  Tick count shifted right 11 bits (2048) */
		uitemp = ( ltY.ulltime >> ALR_INC_ORDER );	

		/* We encounter a zeroed entry, it is better not to pass the epoch shifted value */
		if (uitemp > 0)
		{
			/* Get epoch out of our hokey scheme for saving a byte to the 'ctime' routine basis */
			uitemp += PODTIMEEPOCH;		// Adjust for shifted epoch
		...
		/* Output offset number and time in converted ascii */

		if (USART1_txint_busy() != 0) return;// Return: all buffers full

		// '^' is the first char of the line that signals the PC that this is the response to the "k 1 nnnn" command
		printf ("^LT %4u %11u %s\r",launchtime_yyyy, uitemp, ctime((const time_t*)&uitemp)); USART1_txint_send();
		p1_PC_monitor_readout_launchtimes_reset();
		break;

	case '2': // Command (ascii '2') = readout entry (-yyyy), where yyyy is 0 - n, (going backwards in time)
		switch (launch_local_state)
		{
		case 0:
			
			if (p1_launchtime_set_start(&ltX,launchtime_yyyy) < 0) // Get PID and set readout pointer
			{ // Here something went wrong.
				printf (" Launchtime seek error %4u\n\r", launchtime_yyyy); USART1_txint_send();
				launchtime_sub_cmd = 0;	// So we don't fling 1000's of these messages out
				break;
			}
			/* Compute time limit for readout */
			launchtime_limit = (ltX.ulltime >> 11) - LAUNCHREADOUTMAXSECS;
			launch_local_state = 1;	
			cUpDn = 0;	// Reset 'state' for detecting beginning of launch. See 'packet_setup_ad7799' above.
			break;
		case 1:
			USART1_txint_puts(packet_PC_buf); USART1_txint_send();	// Output packet
				
			/* Detect when tension at beginning of launch fell below a threshold */
			if (cUpDn == 3)
			{
				launch_local_state = 2;
			}
			
			/* Terminate readout if it is getting too long */
			if  ((launchtime_limit >= ulpackettime) && (ulpackettime > 100000))
			{ // Here the readout has past the stop time.  Time to end
				launch_local_state = 2;
			}
			break;
		case 2:
			/* This terminating message has '<' as the 1st char which will flag the PC to close the file. */
			USART1_txint_puts((char *)stopmsg); // "< ##### Stop time has been reached at"
			...
			state_PC_handler = 0; 	// Reset 'r' 'd' 'k' 'q' command which causes menu to print
			ulreadbackstart = 0;	// JIC
			state_readout = 0;	// Reset for new command
			p1_PC_monitor_readout_launchtimes_reset();
			break;

		} // end: 'switch (launch_launch_local_state)'
		break;// end: "case '2'" jic another case added

	} // end: 'switch (launchtime_sub_cmd)'
	return;
}
/******************************************************************************
...
int convert_k_command (char * p)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if ( *(p+4+i) != ' ')
		{
			launchtime_yyyy = launchtime_yyyy * 10 + *(p+4+i) - '0';
		}
	}
...
void p1_PC_monitor_readout_launchtimes_reset(void)
{
	launch_local_state = 0;	
	launchtime_sub_cmd = 0;
	cUpDn = 0;

	return;
}
/* =========================== PUSHBUTTON TIME READOUT =================================================== */
/******************************************************************************
...
*****************************************************************************/
#define PUSHBUTTONREADOUTMAXSECS	4	// Number of secs readout for pushbutton
static int pushbuttontime_sub_cmd = 0;
static int pushbutton_local_state = 0;

static unsigned int pushbuttontime_limit;	// end time for readout of pushbutton
static int pushbuttontime_yyyy;	// Offset number (from PC) into table of pushbutton times
 ...
 */
	...
	switch (pushbuttontime_sub_cmd)
	{
	case '1': // Command (ascii '1') = get entry (-yyyy), where yyyy is 0 - n, (going backwards in time)
		ptY = p1_pushbuttontime_get_entry(pushbuttontime_yyyy);
		/* Linux format time.  Tick count shifted right 11 bits (2048) */
		uitemp = ( ptY.ulltime >> ALR_INC_ORDER );	

		/* We encounter a zeroed entry, it is better not to pass the epoch shifted value (less confusing) */
		if (uitemp > 0)
		{
			/* Get epoch out of our hokey scheme for saving a byte to the 'ctime' routine basis */
			uitemp += PODTIMEEPOCH;		// Adjust for shifted epoch
		...
		// '^' is the first char of the line that signals the PC that this is the response to the "q 1 nnnn" command
		printf ("^PB %4u %11u %s\r",pushbuttontime_yyyy, uitemp, ctime((const time_t*)&uitemp)); USART1_txint_send();
		p1_PC_monitor_readout_pushbuttontimes_reset();
		break;

	case '2':
		switch (pushbutton_local_state)
		{
		case 0:
			
			if (p1_pushbuttontime_set_start(&ptX,pushbuttontime_yyyy) < 0)
			{
				printf (" Pushbuttontime seek error %4u\n\r", pushbuttontime_yyyy); USART1_txint_send();
				pushbuttontime_sub_cmd = 0;	// So we don't fling 1000's of these messages out
				break;
			}
			/* Compute time to end readout */
			pushbuttontime_limit = (ptX.ulltime >> 11) - PUSHBUTTONREADOUTMAXSECS;
			pushbutton_local_state = 1;
			break;
		case 1:
			USART1_txint_puts(packet_PC_buf); USART1_txint_send();//  (
				
			/* Detect when tension at beginning of launch fell below a threshold */
			if (cUpDn == 3)
			{
				pushbutton_local_state = 2;
			}
			
			/* Terminate readout if it is getting too long */
			if  ((pushbuttontime_limit >= ulpackettime) && (ulpackettime > 100000))
			{ // Here the readout has past the stop time.  Time to end
				pushbutton_local_state = 2;
			}
			break;
		...
		} // end: 'switch (pushbutton_local_state)'
		break;// end: "case '2'" jic another case added

	} // end: 'switch (pushbuttontime_sub_cmd)'
	return;
}
/******************************************************************************
...
void p1_PC_monitor_readout_pushbuttontimes_reset(void)
{
	pushbutton_local_state = 0;	
	pushbuttontime_sub_cmd = 0;
	return;
}
/******************************************************************************
...
int convert_q_command (char * p)
{
	...
	/* Convert first field */
	pushbuttontime_sub_cmd = *(p+2);

	/* Convert second field */
	pushbuttontime_yyyy = 0;
	for (i = 0; i < 4; i++)
	{
		if ( *(p+4+i) < '0') return -1;
		if ( *(p+4+i) > '9') return -1;
		...
		if ( *(p+4+i) != ' ')
		{
			pushbuttontime_yyyy = pushbuttontime_yyyy * 10 + *(p+4+i) - '0';
		}
	}
	return 0;	// Return with sucess code.
}
