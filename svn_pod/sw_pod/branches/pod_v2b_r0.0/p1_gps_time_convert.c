...
06-11-2013 Added GPS checksum check (svn_pod Revision: 804)
*/
...
unsigned int gps_time_stampGPRMC(struct TIMESTAMPGP1 *strTS, struct USARTLB strlb, u8 sw)
{
	...
	{ 
		if (cGPStype == GARMIN_18X_5_HZ)
		{
			pp = gps_field_copy_p(&ttmp[0], p, 8);	// Copy time field
			tsize = (pp - p);		// Save size of HHMMSS field
		...
		{ // Here, assuming a u-blox GPS
			...
			if (tsize < 9) 	return 8;	// Too few 'hhmmss.s'
		}
	}
