...
//		}
		...
		if ((cGPS_ready != 0) && ((GPIO_IDR(GPIOD) & (1<<14) ) != 0))
...
//USART1_txint_send();		// Start the line buffer sending
		...
		if ( (uiGPSrtc = gps_time_stamp(&pkt_gps_mn, strlb) ) == 99 )	// (@5) // 5 Hz onboard version
...
//				LED_ctl_turnon(10,100,2);	// Pulse ON width, space width, two flashes
			...
			{ // Here, any initial, dubious readings should be long gone.  Time to use it for sync'ing time.
				...
				if ( rtc_tick_edit(strAlltime.uiTim2diff) == 0)
				{
...
//USART1_txint_send();
				...
				if ( (rtc_tick_edit(strAlltime.uiTim2diff)) == 0) // Precludes a double second count
