...
void LEDonboard_ctl_turnon(unsigned int uiCtob, unsigned int uiSpaceob)
{
	...
	case 0:	// Initial state
		/* Compute systick count when LED should turn off */
		uiLEDob_systick = SYSTICK_getcount32() - (uiCtob*(sysclk_freq/1000));
	...
	case 1: // Waiting to turn LED OFF
		...
		uiLEDob_systick = SYSTICK_getcount32() - (uiSpaceob*(sysclk_freq/1000));

		stateob = 2;
		break;

	case 2:	// LED is off, wait turn it back on
		if (  ((int)((SYSTICK_getcount32() - uiLEDob_systick))) > 0 )//  Time expired? (@2)		
			return;	// Return time not expired.
