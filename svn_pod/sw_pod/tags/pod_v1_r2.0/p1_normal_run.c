...
if ( uiDebug_normC++ > 100000) {uiDebug_normX = 0; uiDebug_normC = 0;}
		...
		/* This polling drives the writing of logging of Pushbutton, GPS, AD7799, accelerometer, battery|thermistor data */
		...
		/* Flash onboard LED so that we know this mode is running when the RS232 is not present */
		LEDonboard_ctl_turnon(25, 1000);	// ON time, OFF time (ms)
