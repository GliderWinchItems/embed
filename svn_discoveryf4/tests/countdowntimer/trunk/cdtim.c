...
void red_led (struct SWITCHDEBPARAMS* p);
void blue_led (struct SWITCHDEBPARAMS* p);
...
// PA0 is the Discovery F4 on-board switch (Blue sw) 
struct SWITCHDEBPARAMS my_pushbutton = {\
	0,  /* u32	flag;		 Debounced switch flag */\
	0,  /* u32	topen;		 Working counter for timing open debounce */\
	0,  /* u32	tclose;		 Working counter for timing close debounce */\
	4000, /* u32	bnco;		 Debounce time open (1/2 ms ticks) test timing: 2 sec delay when pressing switch */\
	2000, /* u32	bncc;		 Debounce time close (1/2 ms ticks) test timing: 1 sec dekat when releasing switch */\
	(u32*)GPIOA, /* u32	port;	 Address of gpio port */\
	0,  /* u32	pin;		 Pin on port (0-31) */\
	2,  /* u32	cbx;		 Callback is made when flag: 0 = 1->0; 1 = 0->1; 2 = either change  */\
	(void*)&blue_led, /* void (*bncb)(void)	 Callback function pointer */\
	0,  /* U32	updn;		 Use internal pull up/dn or none (0) */\
	0,  /* struct SWITCHDEBOUNCER*	next;	 Pointer to next struct */\
};

// External switch (spi2 parallel-serial)
struct SWITCHDEBPARAMS ext_pushbutton1 = {\
	0,  /* u32	flag;		 Debounced switch flag */\
	0,  /* u32	topen;		 Working counter for timing open debounce */\
	0,  /* u32	tclose;		 Working counter for timing close debounce */\
	15, /* u32	bnco;		 Debounce time open (1/2 ms ticks) */\
	50, /* u32	bncc;		 Debounce time close (1/2 ms ticks) */\
	(u32*)&swdbin, /* u32	port;	 Address of in-memory byte */\
	0,  /* u32	pin;		 Pin in byte (0-7) */\
	1,  /* u32	cbx;		 Callback is made when flag: 0 = 1->0; 1 = 0->1; 2 = either change  */\
	(void*)&blue_led, /* void (*bncb)(void)	 Callback function pointer */\
	0,  /* U32	updn;		 Use internal pull up/dn or none (0) */\
	0,  /* struct SWITCHDEBOUNCER*	next;	 Pointer to next struct */\
};
...
/* ---------------------- Let the hapless Op know it is alive ------------------------------------ */
	...
	xprintf(UXPRT,  " \n\rF4 DISCOVERY: CDTIM (Test countdown timer and switch debounce) 01-17-2-14 v0 ....\n\r");
...
/* --------------------- Initialize switchdebouncing ------------------------------------------------------------------- */
	debouncesw_init(swdbout, swdbin, SPI2BUFFSIZE);	// SPI out/in buffers
/* --------------------- Add pushbuttons to be debounced --------------------------------------------------------------- */
	debouncesw_add(&my_pushbutton);	// Add on-board Discovery F4 pushbutton on port A pin 0
/* --------------------- Initialize timer ------------------------------------------------------------------------------ */
