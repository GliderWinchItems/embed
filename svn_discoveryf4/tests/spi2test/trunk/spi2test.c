...
*******************************************************************************/
/*
02-06-2014 changed to UART6.  Debugged spi2rw.c
02-06-2014 rev 190 'spi2rwB' version: Added TX to keep tx data register loaded


F4 plastic clamshell setup for 'scoping SPI2--
6 pin header--
1 GND	blk  ground
2 PB12 	wht  /CS
3 PB13	grn  SCK
...
/* ---------------------- Let the hapless Op know it is alive ------------------------------------ */
	...
	xprintf(UXPRT,  " \n\rDISCOVERY F4 SPI2TEST with SPI2RWB: 02-06-2014  v0\n\r");
...
#define PACECOUNT (168000000/5);	// Pace the output loop
u32	t_led = *(volatile unsigned int *)0xE0001004 + PACECOUNT;
#define SPI2SIZE 3 	// Number of bytes in the transfer	
char bout[SPI2SIZE] = {0x81,0x7e,0xbd};	// Initial outgoing pattern
char bin[SPI2SIZE];
void printbits(char* p);
int x = 0;
u32	t_spi;
u32	t_diff = *(volatile unsigned int *)0xE0001004;;
   ...
   printf and loop overhead.  The SPI update is approximately 50 us per byte. */
	...
	while(1==1)
	{
		...
		if ( ( (int)(*(volatile unsigned int *)0xE0001004 - t_led) ) > 0)
		{
			toggle_4leds(); 
			t_led += PACECOUNT;
			if (spi2_busyB() != 0) // Is SPI2 busy?
			{ // Here, no.  
				
				t_spi = *(volatile unsigned int *)0xE0001004;
				spi2_rwB(bout, bin, SPI2SIZE); // Send/rcv three bytes
				while(spi2_busyB() == 0);
				t_diff = *(volatile unsigned int *)0xE0001004  - t_spi;
			}
			xprintf(UXPRT,"%5u %8u ",x++, t_diff);
