...
for (m = 0; m < idx+1; m++)
{
  for (mm = 0; mm < MAXCANCALFIELDS; mm++)
	if (cantable[m].cal[mm].fld > 14) {printf("OOOOH: %d %d %08X %d \n",m,mm,id1,cantable[m].cal[mm].fld); exit (-19);}
			...
			ret = winch_merge_printfpay1errors(pt,linect);	// Check & list any errors
			if (ret == 0)
			{ // Here, 'ixbegin' 'ixend' 'fix' 'sign' are within bounds, so presume the hapless Op succeeded.
				...
				ret = winch_merge_getformat(pt, &buf[3], linect);		// Format to be used
				if (ret < 0) {printf("cantable input error: getformat: ret: %d at line number %d\n",ret, linect);exit(-2);}

				ret = winch_merge_getfielddescription(pt, &buf[3], linect);// Description for this reading
...
int winch_merge_printfpay1errors(struct CANCAL *pt, int linect)
{
	...
	if ((pt->fix  < -2) || (pt->fix > 4)) 		{printf("Payload field error: 'fix' %d is out of range at line %d\n",pt->fix,linect); err |= 16;}
	if ((pt->sign < -2) || (pt->sign > 2)) 		{printf("Payload field error: 'sign' %d is out of range at line %d\n",pt->fix,linect); err |= 32;}
	return err;
}
/* *************************************************************************************************************
