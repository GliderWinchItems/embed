...
10/14/2013 Hacked for Discovery F4.
*/
/*
STM32407xT6--CAN1,2
Port can# LQFP100 pin number/Head pin number
PA11 CAN1 Rx 70 (used for USB)
PA12 CAN1 Tx 71 (used for USB)

PB05 CAN2 Rx 91 P2|26
PB06 CAN2 Tx 92 P2|23 (used for Audio SCL)

PB08 CAN1 Rx 95 P2|19
PB09 CAN1 Tx 96 P2|20 (used for Audio SDA)

PB12 CAN2 Rx 51 P1|36
PB13 CAN2 Tx 52 P1|37

PD00 CAN1 Rx 81 P2|36
PD01 CAN1 Tx 82 P2|33

This routine does NOT deal with CAN2
USB makes PA11,12 not available

*/

/* 
Page numbers: Ref Manual RM0090 rev 5
...
/* Error counters */
u32 canwinch_ldr_buffoverrun0   = 0;	// Low  priority CAN msg buffer overrun
u32 canwinch_ldr_buffoverrun1   = 0;	// High priority CAN msg buffer overrun
...
/* Transmit buffering */
#define CANXMTBUFSIZE	16		// Number of message that can be queued for sending
...
//	RCC_APB2ENR |= 0x7c;	// Enable ports A thru E
...
const struct PINCONFIG	outputpp = { \
 ...
 * @return	: 0 = no room to add; 1 = room available
 ******************************************************************************/
int can_msg_txchkbuff_ldr(void)
{
	int temp = canxmtIDXm;
	...
	temp += 1; if (temp >= CANXMTBUFSIZE) temp = 0;
	if (temp == canxmtIDXi) return 0; // No room
 ...
 * @return	: 0 = buffer full; 1 = one added to buffer
 * ==> NOTE: dlc is expected to be set when this routine is entered.
 ******************************************************************************/
int can_msg_put_ldr(struct CANRCVBUF *px)
{
	if (can_msg_txchkbuff_ldr() == 0) return 0; // Return if no room for addition
...
can_debugP += 1;	// Msgs added to buffer
	...
	return 1; 	// Return number msg remaining buffered
}
...
//	if ( (canbuf[canbufIDXi].id & (CAN_UNITID_MASK | CAN_DATATYPE_MASK)) == ( myunitid_local | (CAN_EXTRID_DATA_CMD << CAN_EXTRID_SHIFT)) )
...
//	}
	...
	temp = adv_index(canbufIDXi, CANRCVBUFSIZE);
	if (temp != canbufIDXm) // This index advance overrun?
	{ // Here, no, we are not going to overrun the buffer
		canbufIDXi = temp;
	}
	else
	{ // Here yes. Let count the times
		canwinch_ldr_buffoverrun0 += 1; //
	}		
...
//	if ( cantimbuf[canbuftimIDXi].R.id == ( (CAN_RESETALL << CAN_UNITID_SHIFT) | (CAN_DATAID_MASTERRESET << CAN_DATAID_SHIFT) ) )
//	{
//		SCB_AIRCR = (0x5FA << 16) | SCB_AIRCR_SYSRESETREQ;	// Cause a RESET
//	}
	...
	/* Advance index in circular buffer */
	temp = adv_index(canbuftimIDXi, CANRCVTIMBUFSIZE);
	if (temp != canbuftimIDXm) // This index advance overrun?
	{ // Here, no, we are not going to overrun the buffer
		canbuftimIDXi = temp;
	}
	else
	{ // Here yes. Let count the times
		canwinch_ldr_buffoverrun1 += 1; //
	}
